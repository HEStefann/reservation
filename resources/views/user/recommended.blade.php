        ...
        @if ($recommendedRestaurants->count() > 0)
            <div class="space-y-[28px]">
                @foreach ($recommendedRestaurants as $restaurant)
                    <a href="{{ route('user.restaurant', ['restaurant' => $restaurant->title]) }}" class="flex flex-col rounded-bl-2xl rounded-t-2xl rounded-br-2xl bg-white pb-[8px] shadow-2xl">
                        <div class="relative">
                            @if (Auth::check())
                        ...
                        <div class="flex justify-between ml-[25px] mr-[16px] items-center">
                            <p class="text-[8px] font-light text-left text-[#6b686b]">{{ $restaurant->average_price }}$
                                average
                                price
                            </p>
                            <div
